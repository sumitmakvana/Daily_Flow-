...
CREATE OR REPLACE FUNCTION public.manager_visible_team_ids(_user uuid)
RETURNS SETOF uuid
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  WITH RECURSIVE roots AS (
    SELECT id FROM public.teams WHERE manager_id = _user
  ),
  tree AS (
    SELECT id FROM roots
    UNION
    SELECT t.id FROM public.teams t JOIN tree ON t.parent_team_id = tree.id
  ),
  resolved AS (
    SELECT id FROM tree
    UNION
    SELECT id FROM public.teams WHERE public.has_role(_user, 'admin')
  )
  SELECT DISTINCT id FROM resolved;
$$;
...
BEGIN
  ...
  base_users AS (
    SELECT DISTINCT assigned_to AS user_id FROM base_tasks WHERE assigned_to IS NOT NULL
  ),
  exec_kpi AS (
    SELECT
  ...
  workload_rows AS (
    SELECT
      bu.user_id,
      p.display_name,
      COALESCE(SUM(bt.planned_hours), 0) AS planned
      FROM base_users bu
  ...
  auto_runs AS (
    SELECT * FROM public.automation_runs WHERE started_at >= since_ts
  ),
  auto_trend AS (
    SELECT d::date AS day,
  ...
  SELECT jsonb_build_object(
    ...
    'risk', jsonb_build_object(
      ...
      'critical', (SELECT COALESCE(jsonb_agg(to_jsonb(r) ORDER BY r.severity DESC), '[]'::jsonb)
                   FROM (SELECT id, severity, summary, entity_type, entity_id FROM open_risks
                          WHERE severity IN ('high','critical') ORDER BY severity DESC LIMIT 10) r)
    ),
    ...
    'discipline', jsonb_build_object(
      ...
      'blocked_3d', (SELECT COUNT(*) FROM base_tasks WHERE status='Blocked' AND blocked_at < now() - interval '3 days'),
    ...
    'automation', jsonb_build_object(
      ...
      'success_rate', CASE WHEN (SELECT COUNT(*) FROM auto_runs) > 0
                           THEN ROUND((SELECT COUNT(*) FROM auto_runs WHERE status IN ('succeeded','success'))::numeric
                                      / (SELECT COUNT(*) FROM auto_runs) * 100)::int
                           ELSE NULL END,
      'failed_24h', (SELECT failed_24h FROM auto_24h),
      'dead_24h', (SELECT dead_24h FROM auto_24h),
      'pending', COALESCE((SELECT pending_count FROM public.automation_queue_stats LIMIT 1), 0),
      ...
      'follow_ups', (SELECT follow_ups FROM auto_kinds),
      'approvals_auto', (SELECT approvals_auto FROM auto_kinds),
      'escalations', (SELECT escalations FROM auto_kinds),
    ...
    'manager_effectiveness', (
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
        'manager', manager,
        'completion', CASE WHEN total > 0 THEN ROUND(done::numeric/total*100)::int ELSE 0 END,
        'overdue', overdue,
        'appr', COALESCE(ROUND(appr_hours::numeric, 1), 0)
      ) ORDER BY (CASE WHEN total > 0 THEN done::numeric/total ELSE 0 END) DESC), '[]'::jsonb)
      FROM mgr_eff
    ),
    'insights_inputs', jsonb_build_object(
      'cf_last_7d', (SELECT COUNT(*) FROM public.carry_forward_events
                      WHERE created_at >= now() - interval '7 days'
                        AND EXISTS (SELECT 1 FROM base_tasks bt WHERE bt.id = task_id)),
      'cf_prev_7d', (SELECT COUNT(*) FROM public.carry_forward_events
...
CREATE INDEX IF NOT EXISTS idx_tasks_team_due ON public.tasks(team_id, due_date);
CREATE INDEX IF NOT EXISTS idx_tasks_completed_at ON public.tasks(completed_at);
CREATE INDEX IF NOT EXISTS idx_carry_forward_events_created_at ON public.carry_forward_events(created_at);
CREATE INDEX IF NOT EXISTS idx_automation_runs_started_at ON public.automation_runs(started_at);
CREATE INDEX IF NOT EXISTS idx_eod_checkins_date_user ON public.eod_checkins(checkin_date, user_id);
CREATE INDEX IF NOT EXISTS idx_adoption_daily_date_user ON public.adoption_daily(rollup_date, user_id);
