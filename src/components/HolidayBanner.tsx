...
export function HolidayBanner() {
  ...
  if (customAnnouncement) {
    ...
    // Default graphic is just the emoji
    graphicElement = (
  ...
  return (
    <div className={`${maxWidthClass} mx-auto px-3 md:px-4 mt-4 mb-2`}>
      <div className={`relative overflow-hidden bg-gradient-to-r ${themeClass} px-4 py-2.5 rounded-xl shadow-md border border-white/5 flex items-center justify-between transition-all duration-300 z-30`}>
        ...
        <div className="flex-1 flex items-center justify-center gap-3 sm:gap-4 text-left max-w-5xl mx-auto z-10 relative py-0.5">
          <div className="flex-shrink-0 drop-shadow-sm">
            {customImageUrl ? (
              <img src={customImageUrl} alt={holiday?.name || customAnnouncement?.title} className="w-12 h-9 object-cover rounded border border-white/20" />
            ) : (
              graphicElement
            )}
          </div>
          <div className="flex flex-col min-w-0">
            <span className={`text-xs sm:text-sm font-semibold tracking-wide leading-normal ${isTricolor ? 'text-blue-950' : 'text-white'}`}>
              {message}
            </span>
            <span className={`text-[10px] sm:text-[11px] font-medium mt-0.5 opacity-85 ${isTricolor ? 'text-blue-900/80' : 'text-white/80'}`}>
        ...
        <button
          ...
          className={`flex-shrink-0 rounded-full p-1.5 transition-colors cursor-pointer ml-3 z-10 relative ${isTricolor ? 'text-blue-950 hover:bg-black/5' : 'text-white/80 hover:text-white hover:bg-white/10'}`}
          title="Dismiss"
        >
